...
class CsvReader[+T <: JobConfig](sparkSession: SparkSession, config: T, configFile: Parser) extends {
  private var inputSource = ""
  private var readerOptions: Map[String, String] = Map.empty
} with BaseReader[T](sparkSession, config, configFile) {
    ...
    this.configFile.getJsonFields() headOption match {
      ...
      case Some(_) => {
        ...
        this.configFile.getJsonFields().foreach(f => {
          finalDF = finalDF.withColumn(f, functions.from_json(df(f), configFile.getSchemaPerJsonField(f)))
        })
        Some(finalDF)
      }
    }
